...
public class ComplainService {
	...
	@GET
	@Path("/{search}/searchcomplains") // complain end point
	@Produces(MediaType.APPLICATION_JSON)
	public String searchComplains(@PathParam("search") String search) {
		ComplainModel em = new ComplainModel();
		// String output = em.searchEmployees(name);
		
		ArrayList<Complain> al = em.searchComplainsJson(search);
		Gson gson = new Gson();
